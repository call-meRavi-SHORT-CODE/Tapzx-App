...
const ProfileLandingPage: React.FC<ProfileLandingPageProps> = ({ navigation }) => {
  // GET ROUTE PARAMETERS FOR SCANNED USER DATA
  const params = useLocalSearchParams()

  // Extract user data from params or use defaults
  const userData = {
    id: (params.userId as string) || "default_user",
    name: (params.userName as string) || "Ravi",
    title: (params.userTitle as string) || "Business Owner",
    company: (params.userCompany as string) || "Tech Company",
    image:
      (params.userImage as string) ||
      "https://t3.ftcdn.net/jpg/04/19/88/96/360_F_419889684_ZcIYZWwxtns7Q469DcLVybs94GIHNHjm.jpg",
    location: (params.userLocation as string) || "Chennai, Tamil nadu, India",
    verified: params.userVerified === "true" || false,
    isScannedProfile: params.isScannedProfile === "true" || false,
  }
  ...
  const handleExportPress = (): void => {
    Alert.alert("Export Profile 📤", "Choose export format for your profile data", [
      { text: "Cancel", style: "cancel" },
      { text: "Export as PDF", onPress: () => console.log("Export PDF") },
      { text: "Export as vCard", onPress: () => console.log("Export vCard") },
      { text: "Share QR Code", onPress: () => console.log("Share QR") },
    ])
  }

  const handleBackPress = (): void => {
    if (userData.isScannedProfile) {
      // Go back to scan page if viewing scanned profile
  ...
  const handleShareProfile = () => {
    Alert.alert("Share Profile", `Share ${userData.name}'s profile with others`, [{ text: "OK" }])
  }

  // Dynamic about text based on user
  const getAboutText = () => {
    if (userData.isScannedProfile) {
      return `${userData.name} is a ${userData.title} at ${userData.company}. Connect to learn more about their professional journey and expertise.`
    }
    return "AI & ML enthusiast, CSE-AIML undergrad, passionate about building real-world tech solutions. Fast learner, focused, and driven to innovate."
  }
  ...
  const renderFloatingElement = (animValue: Animated.Value, style: ViewStyle, icon: keyof typeof Ionicons.glyphMap) => (
    <Animated.View
      style={[
  ...
  const renderSocialIcon = (iconData: IconData, index: number) => {
    const IconComponent =
      iconData.type === "Ionicons" ? Ionicons : iconData.type === "MaterialIcons" ? MaterialIcons : FontAwesome
    ...
    return (
      <Animated.View
        ...
        style={[
          ...
          {
            transform: [{ translateY: bounceTransform }, { scale: scaleTransform }, { rotate: rotateTransform }],
          },
        ]}
      >
        <TouchableOpacity
          ...
          activeOpacity={0.8}
          onPress={() => console.log(`Pressed ${iconData.name} for ${userData.name}`)}
        >
          <LinearGradient
          ...
          >
            <IconComponent name={iconData.name as any} size={28} color={iconData.color} />
          </LinearGradient>
        </TouchableOpacity>
      </Animated.View>
    )
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Animated.View style={[styles.animatedHeader, { opacity: headerOpacity }]}>
        <BlurView intensity={80} style={styles.headerBlur}>
          <View style={styles.headerContent}>
            ...
            <TouchableOpacity onPress={handleExportPress} style={styles.headerButton}>
      ...
      <View style={styles.fixedHeader}>
        ...
        <TouchableOpacity onPress={handleExportPress} style={styles.floatingHeaderButton}>
          <BlurView intensity={50} style={styles.buttonBlur}>
            <Ionicons name="download-outline" size={24} color="white" />
          </BlurView>
        </TouchableOpacity>
      ...
      >
        ...
        >
          <Image source={{ uri: userData.image }} style={styles.profileImage} resizeMode="cover" />
        </Animated.View>
        ...
        >
          ...
          <View style={styles.profileInfo}>
            ...
            <Text style={styles.role}>{userData.title}</Text>
            {userData.company && <Text style={styles.company}>{userData.company}</Text>}
            <Text style={styles.location}>{userData.location}</Text>

            {userData.isScannedProfile && (
              <View style={styles.scannedBadge}>
                <Ionicons name="scan-circle" size={16} color="#3B82F6" />
                <Text style={styles.scannedText}>Scanned Profile</Text>
              </View>
            )}
          </View>
          ...
          <View style={styles.socialContainer}>
            <Text style={styles.sectionTitle}>Connect with {userData.isScannedProfile ? userData.name : "me"}</Text>
            <View style={styles.socialGrid}>{socialIcons.map((icon, index) => renderSocialIcon(icon, index))}</View>
          </View>
          ...
          <View style={styles.ratingContainer}>
            ...
            >
              <Text style={styles.ratingTitle}>About {userData.isScannedProfile ? userData.name : "Me"}</Text>
